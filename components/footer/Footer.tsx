  ...
  return (
    <SectionWraper>
      <div className=" w-full p-4 max-lg:px-2 max-sm:px-4">
        <FooterList>
          <div className="col-span-2 max-sm:col-span-1  max-md:pr-0">
            <div className="font-clash text-3xl font-bold mb-6 ">JBProMend</div>
            <p className="flex flex-col gap-10 max-sm:text-sm pr-3">
              <span>
                JBProMend stands at the forefront of technology innovation,
                serving as a pivotal bridge that effortlessly connects users
                with dependable local repair services. Let&apos;s help you with your task.
              </span>
              <span>
          ...
          {footer_links.map((item, i) => (
            <div
              className="font-montserrat text-sm md:px-2 md:mx-auto max-sm:my-4"
              key={item.id}
            >
              {/* mobile view */}
              <div
                onClick={() => handler(i)}
                className="hidden cursor-pointer max-sm:flex justify-between "
              >
              ...
              <h3 className="font-clash  max-sm:hidden font-bold text-[16px]">
                {" "}
                {item.header}
              </h3>
              <p
                className={`space-y-3  my-5 max-sm:px-2 overflow-hidden duration-500 ease-in-out transition-all ${
                  collapse === i
                    ? "max-sm:grid max-sm:grid-col-1 grid-rows-1"
                    : "max-sm:grid-rows-[_0fr] max-sm:hidden"
                }`}
              >
                {item.list.map((list, index) => (
                  <span key={list.trim()} className="block text-[#CDD0DD]">
                    {list}
                  </span>
                ))}
              </p>
      ...
      <div className="text-center pt-4 pb-5 relative">
        ...
        <span>All rights reserved. © JBProMend Ltd.</span>
      </div>
    </SectionWraper>
  );
};
