...
const SearchSectionWrapper = tw.div<ButtonProp>`${(p)=> (p.$bool ? "max-sm:hidden" : "sm:hidden block")} bg-white z-50 p-4 rounded-md max-md:w-11/12 w-7/12  max-sm:w-full`;
...
const HelperButtonWrapper = tw.div`flex flex-wrap max-sm:w-11/12 max-sm:justify-center my-2 max-sm:mx-auto`;
